...
extern "C"
{
	 ...
	 * @param modifier The modifier key that was pressed, 0 if no key was
	 * pressed.
	 */
	...
	typedef enum mlx_errno
	{
		...
		MLX_INVIMG,		 // The provided image is invalid, might indicate
						 // mismanagement of images.
	...
	// Set these values, if necessary, before calling `mlx_init` as they define
	// the behaviour of MLX42.
	 ...
	 *
	 * WARNING: The function is called every frame during which the window is
	 * being resized, be aware!
	 *
	 ...
	 *
	 * Under GLFW they are named GLFWcursor and have a wrapper for each
	 * implementation. You can find the ACTUAL cursor in the following files at
	 * GLFW named under *_platform.h
	 */
	 ...
	 * @param[in] value Settings value to determine the state of the setting.
	 * Can be a boolean or an enum / macro.
	 */
	 ...
	 * This is not the same as terminate, this simply tells MLX to close the
	 * window.
	 *
	 ...
	 * mlx_close_window is called, meaning it will loop until the user requests
	 ...
	 *
	 * @note In MacOS this function does nothing, you should use the bundles
	 * icon to set the dock bar icon.
	 ...
	 *
	 * @param[in] index Normally 0, in case of multiple windows, can be
	 * specified
	 * @param[in] width The width of the window.
	 * @param[in] height The height of the window.
	 */
	 ...
	 *
	 * @param[in] mlx The MLX instance handle.
	 * @param[in] cursor The cursor object to display, if null default cursor is
	 * selected.
	 */
	...
	/**
	 * This function sets the key callback, which is called when a key is
	 * pressed on the keyboard. Useful for single keypress detection.
	 *
	 * @param[in] mlx The MLX instance handle.
	...
	/**
	 * This function sets the close callback, which is called in attempt to
	 * close the window device such as a close window widget used in the window
	 * bar.
	 *
	 * @param[in] mlx The MLX instance handle.
	...
	/**
	 * This function sets the resize callback, which is called when the window
	 * is resized
	 *
	 * @param[in] mlx The MLX instance handle.
	 ...
	 * all instances it might have. Additionally, just as extra measures sets
	 * all the data to NULL.
	 *
	 ...
	 *
	 * NOTE: Keep in mind that images that are on the same Z layer cut each
	 * other off. so if you don't see your image anymore make sure it's not
	 * conflicting by being on the same layer as another image.
	 *
